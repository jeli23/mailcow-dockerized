...
cat <<EOF > /opt/postfix/conf/sql/mysql-tls.cnf
[client]
ssl-ca = ${DBSSL_CA}
...
while ! mariadb-admin status -h ${DBHOST} -P ${DBPORT} --ssl-ca=${DBSSL_CA} --ssl-verify-server-cert -u${DBUSER} -p${DBPASS} --silent; do
...
hosts = ${DBHOST}:${DBPORT}
dbname = ${DBNAME}
option_file = /opt/postfix/conf/sql/mysql-tls.cnf
option_group = client
