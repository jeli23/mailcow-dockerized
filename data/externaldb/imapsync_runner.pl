...
$run_dir="/tmp";
if ($ENV{'DBHOST'}) {
  # External database: TLS is required; old DBD::mysql cannot enforce TLS, so
  # require DBD::MariaDB and exit gracefully when it is not available.
  eval { require DBD::MariaDB; 1 } or do {
    print "imapsync runner disabled: DBD::MariaDB is not installed, cannot reach external database over TLS\n";
    exit 0;
  };
  $dsn = 'DBI:MariaDB:database=' . $ENV{'DBNAME'} . ';host=' . $ENV{'DBHOST'} . ';port=' . ($ENV{'DBPORT'} || 3306)
    . ';mariadb_ssl=1;mariadb_ssl_ca_file=' . $ENV{'DBSSL_CA'} . ';mariadb_ssl_verify_server_cert=1';
} else {
