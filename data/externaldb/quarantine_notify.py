...
try:
  ...
  def query_mysql(query, params = None, headers = True, update = False):
    while True:
      try:
        cnx = MySQLdb.connect(user=os.environ.get('DBUSER'), password=os.environ.get('DBPASS'), database=os.environ.get('DBNAME'), host=os.environ.get('DBHOST') or 'localhost', port=int(os.environ.get('DBPORT') or 3306), ssl={'ca': os.environ.get('DBSSL_CA')} if os.environ.get('DBSSL_CA') else None, charset="utf8mb4", collation="utf8mb4_general_ci")
      except Exception as ex:
        print('%s - trying again...'  % (ex))
        time.sleep(3)
      else:
