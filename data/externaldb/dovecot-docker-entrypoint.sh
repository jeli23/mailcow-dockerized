...
while ! mariadb-admin status ${MYSQL_TLS_OPTS} -u${DBUSER} -p${DBPASS} --silent; do
...
# Connection string for generated Dovecot SQL configs (TCP + TLS)
DOVECOT_SQL_CONNECT="host=${DBHOST} port=${DBPORT} dbname=${DBNAME} user=${DBUSER} password=${DBPASS} ssl_ca=${DBSSL_CA} ssl_verify_server_cert=yes"
...
IMAPSYNC_TABLE=$(mariadb ${MYSQL_TLS_OPTS} -u ${DBUSER} -p${DBPASS} ${DBNAME} -e "SHOW TABLES LIKE 'imapsync'" -Bs)
[[ ! -z ${IMAPSYNC_TABLE} ]] && mariadb ${MYSQL_TLS_OPTS} -u ${DBUSER} -p${DBPASS} ${DBNAME} -e "UPDATE imapsync SET is_running='0'"
...
while [[ ${VERSIONS_OK} != 'OK' ]]; do
  if [[ ! -z $(mariadb ${MYSQL_TLS_OPTS} -u ${DBUSER} -p${DBPASS} ${DBNAME} -B -e "SELECT 'OK' FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = \"${DBNAME}\" AND TABLE_NAME = 'versions'") ]]; then
    VERSIONS_OK=OK
  else
...
if [ -f ${PUBKEY_MCRYPT} ]; then
  ...
  if [ ${#GUID} -eq 64 ]; then
    mariadb ${MYSQL_TLS_OPTS} -u ${DBUSER} -p${DBPASS} ${DBNAME} << EOF
