...
while ! /usr/bin/mariadb-admin status ${MYSQL_TLS_OPTS} -u${DBUSER} -p${DBPASS} --silent > /dev/null; do
  sleep 2
done
log_f "Database OK"
...
while [[ -z ${DOMAIN_TABLE} ]]; do
  ...
  DOMAIN_TABLE=$(mariadb ${MYSQL_TLS_OPTS} -u ${DBUSER} -p${DBPASS} ${DBNAME} -e "SHOW TABLES LIKE 'domain'" -Bs)
...
while true; do
  ...
  SQL_DOMAINS=$(mariadb ${MYSQL_TLS_OPTS} -u ${DBUSER} -p${DBPASS} ${DBNAME} -e "SELECT domain FROM domain WHERE backupmx=0 and active=1" -Bs)
  if [[ ! $? -eq 0 ]]; then
  ...
  if [[ ${AUTODISCOVER_SAN} == "y" ]]; then
    SQL_MTA_STS_DOMAINS=$(mariadb ${MYSQL_TLS_OPTS} -u ${DBUSER} -p${DBPASS} ${DBNAME} -e "SELECT domain FROM mta_sts WHERE active = 1" -Bs)
    if [[ $? -eq 0 ]]; then
  ...
  if [[ ${AUTODISCOVER_SAN} == "y" ]]; then
    SQL_ALIAS_DOMAINS=$(mariadb ${MYSQL_TLS_OPTS} -u ${DBUSER} -p${DBPASS} ${DBNAME} -e "SELECT ad.alias_domain FROM alias_domain ad INNER JOIN mta_sts m ON ad.target_domain = m.domain WHERE ad.active = 1 AND m.active = 1" -Bs)
    if [[ $? -eq 0 ]]; then
